	...
	MiscLib::Vector< MiscLib::Vector< GfxTL::VectorXD< 2, size_t > > > *polys)
{
	...
	unsigned int edgesExtent[] ={ (unsigned int)uextent + 1, (unsigned int)vextent + 1, (unsigned int)3, (unsigned int)3 };
	...
	do
	{
			...
			|| (!uwrap && !vwrap) || (vwrap && !uwrap && y == 0))
		{
			...
			unsigned int edgeIndex[] = { (unsigned int)x, (unsigned int)y, (unsigned int)1, (unsigned int)2 };
		...
		else if(uwrap && !vwrap && x == 0 && (size_t)y != vextent - 1)
		{
			...
			if(!IsEdge(componentImg, uextent, vextent, label, uwrap, vwrap,
				...
				continue;
			// check if edge was visited already
			unsigned int edgeIndex[] = { (unsigned int)x + 1, (unsigned int)y, (unsigned int)0, (unsigned int)1 };
			if(edges.find(edgeIndex))
				continue;
			// on top of pixel
			prevx = -1;
		...
		else if(uwrap && !vwrap && x == 0 && (size_t)y == vextent - 1)
		{
			...
			// on bottom of pixel
			// check if edge was visited already
			unsigned int edgeIndex[] = { (unsigned int)x + 1, (unsigned int)y + 1, (unsigned int)0, (unsigned int)1 };
			if(edges.find(edgeIndex))
				continue;
			prevx = -1;
			prevy = 0;
		...
		else if(!uwrap && vwrap && (size_t)x == uextent - 1)
		{
			...
			// on bottom of pixel
			// check if edge was visited already
			unsigned int edgeIndex[] = { (unsigned int)x + 1, (unsigned int)y + 1, (unsigned int)1, (unsigned int)0 };
			if(edges.find(edgeIndex))
				continue;
			prevx = 0;
			prevy = 1;
